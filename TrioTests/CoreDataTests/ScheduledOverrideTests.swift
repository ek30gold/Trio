...
///
/// The bug these cover: activation used to be driven by an in-process `Task.sleep` timer armed
/// from the view layer, which only ran while the relevant screen had been opened since app
/// launch. If the app was terminated and relaunched in the background, nothing re-armed it, the
/// scheduled row never activated, and once its start time passed it was unreachable — the
/// "scheduled overnight, did not fire in the morning" failure.
///
...
/// `fetchDueScheduledOverrides(asOf:)` finds past-due Overrides so an app-lifetime service
/// (`ScheduledOverrideManager`) can act on them.
