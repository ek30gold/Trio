...
extension Home.RootView {
    ...
    /// Current insulin delivery rate. Falls back to the max-IOB warning when dosing is capped at
    /// zero, since no rate the loop reports would be actionable in that state.
    @ViewBuilder private var basalRateItem: some View {
        if state.maxIOB == 0.0 {
            HStack {
            ...
            if let apsManager = state.apsManager, !apsManager.isSuspended, apsManager.isScheduledBasal != nil {
                HStack {
                ...
                .accessibilityElement(children: .ignore)
                .accessibilityLabel(Text("Basal rate"))
                .accessibilityValue(Text(
                    basalString ?? String(localized: "No Data", comment: "No basal rate available")
                ))
            }
        }
    }

    /// The rate shown in the meal row: the profile's scheduled rate while the pump runs its own
    /// schedule, otherwise the last temp basal the loop enacted.
    private var basalString: String? {
        var rate: NSNumber = 0
    ...
    private func scheduledBasalDeliveryRate(at when: Date) -> NSNumber? {
