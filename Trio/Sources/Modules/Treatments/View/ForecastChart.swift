...
struct ForecastChart: View {
    ...
    private var forecastChartLabels: some View {
        // The carb and insulin pills show projected carbs and insulin on board, i.e. what will be on
        // board once the pending entry is submitted, not a mirror of the input fields.
        //
        // `state.simulatedDetermination` has two writers: a `simulateDetermineBasal` run, which
        // accounts for the pending entry, and `mapForecastsFromController()`, which supplies the
        // last stored determination and does not. `updateForecasts(with:)` only accepts the latter
        // when no entry is pending, so whenever there is something to fold in these values include
        // it. Falling back to the last real determination (`state.cob`/`state.iob`) keeps the pills
        // showing a real number before any determination has been simulated at all.
