...
- **Creating a new `.swift` file is not enough on its own.** Writing a new file to disk and committing it to git does NOT add it to the Xcode build target. It must also be registered in `Trio.xcodeproj/project.pbxproj` (in the `PBXBuildFile`, `PBXFileReference`, `PBXGroup`, and `PBXSourcesBuildPhase` sections), or the build will fail with "cannot find 'X' in scope" even though the file clearly exists and `grep`/`git log` checks pass. When creating a new file:
  ...
  6. **Exception — synchronized folders (new in v1.0).** The four-section rule applies to plain `PBXGroup` directories. v1.0 migrated some directories to Xcode 16 `PBXFileSystemSynchronizedRootGroup`, where files on disk are picked up automatically and a manual `project.pbxproj` edit is **wrong**. Verified example: `LiveActivity/Views` (group `DDCEBF412CC1B42500DF4C36`, referenced by two targets) — dropping a `.swift` file in there needs no pbxproj change at all. `Preview Content` is also synchronized. By contrast `Trio/Sources/Services/LiveActivity/Data/` is still a plain group and does need the four entries (sibling `GlucoseData.swift` has exactly four). **Before registering any new file, check which kind its directory is:** `grep -n "PBXFileSystemSynchronizedRootGroup" Trio.xcodeproj/project.pbxproj` and see whether the target directory is listed.
